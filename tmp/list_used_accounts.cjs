...
async function check() {
    ...
    try {
        const [rows] = await pool.execute("SELECT data FROM `aubel_srl_asientos`").catch(() => [[]]);
        const used = new Set();
        rows.forEach(r => {
            const data = typeof r.data === 'string' ? JSON.parse(r.data) : r.data;
            (data.detalles || []).forEach(d => used.add(d.cuentaId));
        });
        console.log('Cuentas IDs usadas: ' + Array.from(used).join(', '));
    } catch(e) {
