...
import { Search, Bell, Settings, HelpCircle, User, LogOut, Check, Building2, Calendar } from 'lucide-react';
                ...
                <header className="top-header">
                    <div ref={searchRef} style={{ position: 'relative', width: '400px' }}>
                        ...
                        <input
                            ...
                            placeholder="Buscar en Iubel ERP (Cuentas, Facturas, Contactos...)"
                                ...
                                {showNotifications && (
                                    <div className="dropdown-menu">
                                        <div className="dropdown-header">
                                            <span style={{ fontWeight: 700 }}>Notificaciones (<span>{unreadCount}</span>)</span>
                                            <button style={{ fontSize: '0.75rem', color: 'var(--primary)', fontWeight: 600 }} onClick={(e) => { e.stopPropagation(); clearAll(); }}><span>Limpiar todo</span></button>
                                        </div>
                                        <div style={{ maxHeight: '350px', overflowY: 'auto' }}>
                                            {notifications.length === 0 ? (
                                                <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.875rem' }}><span>No hay notificaciones nuevas</span></div>
                                            ) : (
                                                notifications.map(n => (
                                                    <div
                                                        key={`notif-${n.id}`}
                                                    ...
                                                    >
                                                        ...
                                                        <p style={{ fontSize: '0.8125rem', color: 'var(--text-muted)', margin: 0 }}><span>{n.message}</span></p>
                                                    </div>
                                                ))
                                            )}
                                        </div>
